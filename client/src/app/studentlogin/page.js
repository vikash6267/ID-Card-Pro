...
const StudentLogin = () => {
  ...
  const handleLogin = async () => {
    ...
    try {
      let response= null;
      if (type ==="student") {
         response = await axios.post("/user/student-login", {
          schoolId,
          userName,
          password,
        });
      ...
      if (type ==="staff") {
        response = await axios.post("/user/staff-login", {
         schoolId,
         userName,
         password,
       });
     ...
     type === "student" ?  router.push(
      `https://cardpro.co.in/shareview/edit/${response.data.studentId}?schoolid=${schoolId}`
    ) : router.push(
      `https://cardpro.co.in/shareview/staffedit/${response.data.staffId}?schoolid=${schoolId}`
    )
        ...
        {loading ? (
          ...
          <div className="w-full max-w-md bg-white shadow-lg rounded-xl p-6 sm:p-8">
            ...
            <h2 className="text-3xl font-bold text-center text-gray-800 mb-6">
              {type === "student" ? "Student" : "Staff"} Login
            </h2>
